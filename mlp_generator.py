...
class MLPSearchSpace(object):
    ...
    def vocab_dict(self):
        nodes = [8, 12, 16, 20, 24,28, 32, 40, 48, 56, 64]
        act_funcs = ['swish','relu', 'elu']
        layer_params = []
        layer_id = []
...
class MLPGenerator(MLPSearchSpace):
    ...
    def compile_model(self, model):
        ...
        if self.mlp_loss_func=='focal_loss':
            loss=binary_focal_loss()
        else:
            loss=self.mlp_loss_func
        model.compile(loss=[loss], optimizer=optim, metrics=self.metrics)
        return model
    ...
    def train_model(self, model, x_data, y_data,validation_x,validation_y,weights,nb_epochs, validation_split=0.1, callbacks=None):
        if self.mlp_one_shot:
